...
defineOptions({ name: "NsCheckbox", inheritAttrs: false });
...
const checkboxClasses = computed(() => {
  return {
    ...
    [`ns-checkbox--${checkboxSize.value}`]: checkboxSize.value,
  };
});
...
<template>
  <div class="ns-checkbox" :class="checkboxClasses" @click="handleChange">
    <input
    ...
    />
    <span class="ns-checkbox__inner"></span>
    <span class="ns-checkbox__label" v-if="$slots.default || label">
      <slot>{{ label }}</slot>
    </span>
  </div>
</template>
